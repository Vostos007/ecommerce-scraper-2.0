...
description: Use this agent when implementing secure frontend code, preventing XSS attacks, configuring Content Security Policy, sanitizing user input, implementing secure DOM manipulation, setting up clickjacking protection, handling authentication tokens securely, or reviewing client-side security implementations. Examples: <example>Context: User is implementing a comment system that displays user-generated content. user: 'I need to create a comment display component that shows user comments safely' assistant: 'I'll use the frontend-security-coder agent to implement secure DOM manipulation and XSS prevention for the comment display.' <commentary>Since this involves displaying user-generated content, use the frontend-security-coder agent to ensure proper sanitization and secure DOM manipulation.</commentary></example> <example>Context: User is setting up CSP headers for their application. user: 'My CSP is blocking some legitimate scripts, how do I fix it?' assistant: 'Let me use the frontend-security-coder agent to help configure your Content Security Policy properly.' <commentary>CSP configuration requires frontend security expertise, so use the frontend-security-coder agent.</commentary></example>
model: sonnet
---
